$(document).ready(function () {
   ...
   //         async: false,
        ...
        $.ajax({
            ...
            success: function (vv) {
                ...
                $("#capacity").html('');
                $.each(vv, function (i, states) {
                    $("#capacity").append('<option value="' + states.Value + '">' +
                         states.Text + '</option>');
                    // here we are adding option for States
                    LiftModelCapacityChange();
                });
        ...
        if ($("#LiftType").val() == "Domus Evolution") {
            ...
            //}
            $(FrontEntCarDoorFinish).find('option').filter(function () {
                return this.text === 'Brushed Stainless Steel';
            }).attr('selected', 'selected');
            $(SecondEntCarDoorFinish).find('option').filter(function () {
                return this.text === 'Brushed Stainless Steel';
            }).attr('selected', 'selected');
        }
        else {
    ...
    */
    ...
    $("#LDFEmbDoorClose1").prop("checked", true);
    $("#LDFEmbDoorClose2").prop("checked", true);
    $("#LDFEmbDoorClose3").prop("checked", true);
    $("#LDFEmbDoorClose4").prop("checked", true);
    $("#LDFEmbDoorClose5").prop("checked", true);
    $("#LDFEmbDoorClose6").prop("checked", true);
    $("#LDFEmbDoorClose7").prop("checked", true);
    $("#LDFEmbDoorClose8").prop("checked", true);
    $("#LDFEmbDoorClose9").prop("checked", true);
    $("#LDFEmbDoorClose10").prop("checked", true);
...
function setCarWallsDefault() {
    ...
    setTimeout(function () {
        $(CarWallsRHS).find('option').filter(function () {
            return this.text === 'Skinplate F42PPS brushed stainless steel';
        }).attr('selected', 'selected');
...
function floorsChange() {
    ...
    if ($(NoOfFloors).val() == "2") {
        ...
        if (($("#entranceType").val() == "Front & Rear entrance") && ($("#codeComplence").val() == "Part 15")) {

            $("#Travel").val("4000");
            $("#FtoFDistance1").val("2000");
            $("#FtoFDistance2").val("2000");
...
function LiftModelCapacityChange() {
    ...
    if (!($('#liftModel').val().toLowerCase().indexOf('special') >= 0)) {
        ...
        $('#liftModel').val() == 'RS-2A/4'
            ...
            $("#capacity").html('');
            $("#capacity").append('<option value="' + '300' + '">' +
             '300 Kg' + '</option>');
        }
